...
export const getStockUniverse = cache(async function getStockUniverse(options = {}) {
  ...
  const localItems = query ? searchLocalStockCatalog(query, { limit }).map(buildLocalUniverseItem) : getMockStockUniverse().slice(0, limit);

  return resolveServiceData({
    endpoint: `/v1/stocks/universe${search.size ? `?${search.toString()}` : ""}`,
    buildMockData: () => ({
      items: localItems,
      poolName: query ? "A 股目录检索" : "核心股票池",
      poolSize: query ? localItems.length : getMockStockUniverse().length,
